...
describe('getPathFor', () => {
	...
	test('book documents path', () => {
		expect(getPathFor('book-documents', 'entry')).toBe(
			'/llibre/entry/documents/',
		)
	})
	...
	test('empty entry ID', () => {
		expect(() => getPathFor('article', '')).toThrowError(
			'entryId can’t be empty',
		)
	})
	...
	test('undefined parent entry ID', () => {
		expect(getPathFor('article-tag', 'entry', undefined)).toBe(
			'/articles/entry/',
		)
	})
	...
	test('special characters in entry ID', () => {
		expect(() =>
			getPathFor('article', 'entry_with/special*chars'),
		).toThrowError(
